  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (f) {
      if (!f.name.match(/\.xlsx?$/i)) {
        toast.error('Hanya file .xls atau .xlsx yang didukung')
        return
      }
      setFile(f)
      setResult(null)
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto" style={{ zIndex: 10000 }}>
        ...
        <div className="flex items-center justify-between p-4 pb-3 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-sm font-bold text-slate-800">Upload Data Excel</h2>
              <p className="text-[11px] text-slate-400">Import data ODP dari file Excel</p>
            </div>
          </div>
            ...
            >
              ...
              {file ? (
                ...
                <>
                  ...
                  <div className="text-xs text-slate-500">Klik untuk pilih file .xls atau .xlsx</div>
                  <div className="text-[10px] text-slate-400 mt-1">Format harus sama dengan template ODP List</div>
                </>
              )}
            </div>
          </div>
          ...
          {result && (
            <div className="bg-slate-50 rounded-lg p-3 border border-slate-200">
              ...
              <div className="space-y-1 text-[11px]">
                <div className="flex justify-between"><span className="text-slate-500">Total baris di Excel</span><span className="font-semibold">{result.totalRows?.toLocaleString()}</span></div>
                {mode === 'append' ? (
                  <div className="flex justify-between"><span className="text-slate-500">Ditambah/Diupdate</span><span className="font-semibold text-emerald-600">{result.imported?.toLocaleString()}</span></div>
                ) : (
                  <>
                    <div className="flex justify-between"><span className="text-slate-500">Data lama dihapus</span><span className="font-semibold text-red-600">{result.deleted?.toLocaleString()}</span></div>
                    <div className="flex justify-between"><span className="text-slate-500">Data baru diimport</span><span className="font-semibold text-emerald-600">{result.imported?.toLocaleString()}</span></div>
                  </>
                )}
                <div className="flex justify-between"><span className="text-slate-500">Dilewati (kosong)</span><span className="font-semibold">{result.skipped?.toLocaleString()}</span></div>
